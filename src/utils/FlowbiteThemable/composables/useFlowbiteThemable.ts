...
export function useFlowbiteThemable(
  ...
  const backgroundClasses = computed(() =>
    !themeName.value ? "" : flowbiteThemeClasses[themeName.value].background
  );

  const borderClasses = computed(() =>
    !themeName.value ? "" : flowbiteThemeClasses[themeName.value].border
  );

  const color = computed(() => theme?.value || undefined);

  const disabledClasses = computed(() =>
    !themeName.value ? "" : flowbiteThemeClasses[themeName.value].disabled
  );

  const focusClasses = computed(() =>
    !themeName.value ? "" : flowbiteThemeClasses[themeName.value].focus
  );

  const hoverClasses = computed(() =>
    !themeName.value ? "" : flowbiteThemeClasses[themeName.value].hover
  );

  const textClasses = computed(() =>
    !themeName.value ? "" : flowbiteThemeClasses[themeName.value].text
  );
