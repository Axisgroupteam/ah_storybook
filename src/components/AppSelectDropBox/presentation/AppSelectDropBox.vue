  ...
  <fwb-dropdown @toggleVisibility="handleToggle">
    <template #trigger>
      <fwb-tooltip placement="right" :active="!showDrawer" :distance="16" expanded>
        <template #trigger>
          <div
            class="flex justify-between ring-red-200 dark:ring-red-800 items-center rounded-lg px-[8px] relative bg-red-100 dark:bg-red-900"
            :class="[
              { 'ring-[3px]': opened },
              { 'ring-0': !opened },
              { 'w-[224px] py-3': showDrawer },
          ...
          >
            <div class="flex gap-3 text-black dark:text-white items-center text-center">
              ...
              <span
                v-show="showDrawer"
                :class="!showDrawer ? 'text-transparent' : ' text-neutral-900 dark:text-white'"
    ...
    <ul>
      <li
        ...
        class="px-4 items-center text-xl overflow-x-clip w-[224px] py-4 flex gap-2 border-b whitespace-nowrap border-b-neutral-200 dark:border-b-neutral-600 last:border-b-0 hover:bg-neutral-100 hover:dark:bg-neutral-600 cursor-pointer first:rounded-t-lg justify-start last:rounded-b-lg text-neutral-500 hover:text-neutral-900 dark:hover:text-white dark:text-neutral-400 font-medium"
        @click="selectOption(i)"
      >
        <component :is="getFBIcon(i.icon)" />
        <span>{{ i.title }}</span>
      </li>
    </ul>
  </fwb-dropdown>
...
import FwbTooltip from '../../FwbTooltip/FwbTooltip.vue'
...
const selectedOption = ref(computedOptions.value[0])
...
const handleOptions = (option) => {
  ...
  computedOptions.value.map((el) => {
    if (el != option) auxOptions.value.push(el)
  })
  return auxOptions.value
}

const dropdownOptions = ref(handleOptions(selectedOption.value))

const selectOption = (option) => {
  selectedOption.value = option
  dropdownOptions.value = handleOptions(option)
}
</script>
