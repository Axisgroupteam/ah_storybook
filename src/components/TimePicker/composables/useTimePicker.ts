...
export const useTimePicker = () => {
  ...
  const updateAllColumns = () => {
    ...
    if (selectedPeriod.value)
      periods.value = [
        ...
        ...periods.value.filter((period) => period !== selectedPeriod.value)
      ]
  ...
  const decrementStartIndex = (type: 'hours' | 'minutes' | 'periods') => {
    if (type === 'hours') {
      ...
      periodsStartIndex.value =
        (periodsStartIndex.value - 1 + periods.value.length) % periods.value.length
    }
  }
  ...
  const formatTime = (time: string) => {
    // Utilizamos una expresión regular para extraer horas y minutos
    ...
    if (match) {
      ...
      return { hour: formattedHour, minute: formattedMinute, period }
    } else {
      // Si el formato no es válido, devolvemos valores nulos
      return { hour: null, minute: null, period: null }
    }
  }

  const convertTo24HourFormat = (time12h: string) => {
    // Verificar si el formato es válido (hora:minuto seguido de un espacio y AM/PM)
    const timePattern = /^(0?[1-9]|1[0-2]):([0-5][0-9]) (AM|PM)$/i
    if (!timePattern.test(time12h)) {
      return 'NaN:NaN' // Retornar valor para identificar el error
    }
