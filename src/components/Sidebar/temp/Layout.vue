<template>
  <div class="bg-neutral-100 dark:bg-neutral-900 min-h-full h-screen max-h-screen">
    <div
      class="flex justify-start pl-2 items-center h-[64px] w-full fixed left-0 right-0 top-0 z-30 bg-white border-b dark:border-neutral-700 dark:bg-neutral-800"
    >
    ...
    <main class="h-full p-4 flex pt-20">
...
onClickOutside(target, (event) => {
  ...
  // Verificar si el elemento clickeado es parte del scrollbar o del botón del menú
  if (
    !clickedElement.closest('#module-select') &&
    !clickedElement.closest('.os-scrollbar') &&
    !clickedElement.closest('#drawer-navigation')
  ) {
    if (drawer.value) {
      drawer.value = false
    }
