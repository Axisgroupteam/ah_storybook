<template>
  <div class="w-full flex flex-col justify-start items-center gap-0 rounded-b-lg h-full pb-0">
    <FwbTable
    ...
    >
      <FwbTableHead v-if="showHeader">
        <FwbTableHeadCell v-for="(header, index) in headers" v-show="header.title" :key="index">
          <template #suffix>
      ...
      <FwbTableBody>
        <template v-if="_content.length > 0 && !loading">
          ...
          >
            <FwbTableCell v-for="(header, index) in headers" :key="header.key">
