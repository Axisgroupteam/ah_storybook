...
onMounted(() => {
  const panelIndex = Array.from(panel.value.parentElement.children).indexOf(
    panel.value
  );
  accordionState.value.panels[panelId] = {
    ...
    order: panelIndex,
    isVisible:
      (accordionState.value.openFirstItem && panelIndex === 0) ?? false,
  };
});
onUnmounted(() => {
  if (
    accordionState.value &&
    accordionState.value.panels &&
    accordionState.value.panels[panelId]
  ) {
