...
import type { InputSize, InputType, ValidationStatus } from './types'
...
const inputType = ref(inputTypes.password)
const handleInputTypes = () => {
  if (inputType.value === inputTypes.password) {
    inputType.value = inputTypes.text
  } else {
    inputType.value = inputTypes.password
  }
}
</script>
